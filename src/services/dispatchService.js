...
export const dispatchService = {
  ...
  /** POST /admin/dispatch/bookings/:bookingId/auto-assign */
  async autoAssign(bookingId) {
    return withMockFallback(
      () => apiClient.post(`/admin/dispatch/bookings/${bookingId}/auto-assign`, {}),
      () => {
        const driver = drivers.find((d) => d.status === DRIVER_STATUS.ACTIVE);
        const vehicle = vehicles.find((v) => v.status === VEHICLE_STATUS.AVAILABLE);
        if (!driver || !vehicle) throw Object.assign(new Error('No free driver/vehicle available'), { status: 409 });
        return mockAssign(bookingId, driver.id, vehicle.id);
      }
    );
  },
  ...
  async nearbyDrivers(lat, lng) {
    return withMockFallback(
      () => apiClient.get('/admin/location/nearby', { params: { lat, lng } }),
      () => mockResolve([])
    );
  },
