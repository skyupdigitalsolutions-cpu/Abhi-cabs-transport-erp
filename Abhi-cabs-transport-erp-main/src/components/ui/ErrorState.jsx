...
export default function ErrorState({ message = 'Something went wrong. Please try again.', onRetry }) {
  ...
  if (notBuiltYet) {
    return (
      <div style={wrapper}>
        ...
        {onRetry && (
          <button onClick={onRetry} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 8, fontSize: 13.5, fontWeight: 700, cursor: 'pointer', border: '1.5px solid #E8E8E4', backgroundColor: '#fff', color: '#111', marginTop: 4 }}>
  ...
  return (
    <div style={wrapper}>
      ...
      <p style={{ fontSize: 13.5, color: '#9A9A9A', maxWidth: 360, margin: 0, lineHeight: 1.5 }}>{message}</p>
      {onRetry && (
        <button onClick={onRetry} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 8, fontSize: 13.5, fontWeight: 700, cursor: 'pointer', border: '1.5px solid #E8E8E4', backgroundColor: '#fff', color: '#111', marginTop: 4 }}>
